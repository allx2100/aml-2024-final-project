...
class BetaDiagVAE(BaseVAE):
    def __init__(self, input_size, latent_dim, final_beta = 1, hidden_dims=[512, 256, 128, 64, 32]):
        super(BetaDiagVAE, self).__init__()
    ...
    def loss_function(self, args, time):
        recons = args[0]
        input = args[1]
        mu = args[2]
        log_var = args[3]

        kld_weight = max(time / 100.0 * self.final_beta, self.final_beta)
