import torch
import dill
from models.beta_diag_vae import BetaDiagVAE
from data.utils import *

def train(data_loader, model, loss_fn, optimizer, train_loss, recon_loss, kld_list, time):
    ...
    for i, data in enumerate(data_loader):
        ...
        total_loss = loss_fn(outputs, time)
...
def validate(data_loader, model, loss_fn, val_loss, time):
    running_vloss = 0
    size = len(data_loader.dataset)
    batch_size = data_loader.batch_size
    ...
    with torch.no_grad():
        for i, vdata in enumerate(data_loader):
            ...
            vloss = loss_fn(voutputs, time)['Reconstruction_Loss']
...
model = BetaDiagVAE(3072, 10, final_beta=1e-3)
...
for i in range(300):
    train(train_loader, model, loss_fn, optimizer, train_loss, recon_loss, kld, i+1)
    validate(val_loader, model, loss_fn, val_loss, i+1)
