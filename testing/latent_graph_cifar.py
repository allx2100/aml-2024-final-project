...
anomaly_data_pts = torch.reshape(anomaly_data[100:200], (-1, 3, 32, 32))

anomaly = []
for data_pt in anomaly_data_pts:
    mean = model(data_pt.reshape(1, 3, 32, 32))[2][
        0
    ]  # Note: change to model(data_pt.reshape(1, 3, 32, 32))[5][0] for NF models
    anomaly.append(mean.detach().numpy())

# Load the normal data
load_normal_file = "data/cifar_grayscale_dataloaders.pkl"
loaders = unpickle(load_normal_file)

test_data = loaders["test"].dataset
test_data_pts = torch.reshape(test_data[100:200], (-1, 3, 32, 32))
